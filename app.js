...
document.addEventListener('DOMContentLoaded', function() {
    console.log('📱 EasyTalk Mobile - 핸드폰 전용 버전 로드 완료');
    setupMobileEvents();
    loadSettings();
...
function escapeRegExp(string) {
    return string.replace(/[.*+?^${}()|[\]\\]/g, '\\function generateRandomQuiz() {
    const words = Object.keys(realKoreanDictionary);
    const randomWord =');
}
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
        // 간단한 서비스 워커 (오프라인 캐싱)
        const swCode = `
            const CACHE_NAME = 'easytalk-mobile-v1';
            const urlsToCache = ['/'];
...
console.log('✅ EasyTalk Mobile - 핸드폰 전용 버전 완성!');
console.log('📱 주요 기능:');
